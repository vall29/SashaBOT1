...
🌐️ *URL*: ${url}`
            ...
            try {
                ...
                const ytvh2 = await ytvh.json()
                 if (ytvh2.status == false) {
                    ...
                    if (Number(ytvh2.result.size.split(' MB')[0]) > 75.00) return tobz.reply(from, `Maaf durasi video sudah melebihi batas maksimal 30 MB!`, id)
            ...
            try {
                ...
                const webplay2 = await webplay.json()
                 if (webplay2.status == false) {
                    ...
                    if (Number(webplay2.result.size.split(' MB')[0]) >= 30.00) return tobz.reply(from, 'Maaf durasi music sudah melebihi batas maksimal 10 MB!', id)
            ...
            try {
                ...
                const vhtearyt33 = await vhtearyt3.json()
                 if (vhtearyt33.status == false) {
                    ...
                    if (Number(vhtearyt33.result.size.split(' MB')[0]) >= 50.00) return tobz.reply(from, 'Maaf durasi audio sudah melebihi batas maksimal 10 MB!', id)
        ...
        case `${prefix}video`: // SEARCH VIDEO FROM YOUTUBE
            ...
            tobz.reply(from, 'PREMIUM COMMAND, HUBUNGI : wa.me/6281338888294', id)
            break
        case `${prefix}getvideo'l`: // DOWNLOADER VIDEO FROM #VIDEO
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            tobz.reply(from, 'PREMIUM COMMAND, HUBUNGI : wa.me/6281338888294', id)
            break
...
➸ *Wilayah:* +${region_name}`
   ...
   *Text : ${swrt1}*`
		...
		case `${prefix}nyimak`:
            ...
            break
        case `${prefix}xxx`:
            if (!isVipUser) return tobz.reply(from, `Perintah ini hanya bisa di gunakan oleh Admin, Owner, dan User VIP Sasha!`, id)
            if (!isNsfw) return tobz.reply(from, `command/Perintah NSFW belum di aktifkan di group ini!`, id)
            if (!isGroupMsg) return tobz.reply(from, `Perintah ini hanya bisa di gunakan dalam group!`, id)
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik #limit Untuk Mengecek Kuota Limit Kamu`, id)
        ...
        case `${prefix}getxxx`:
            //if (!isAdmin) return tobz.reply(from, `Perintah ini hanya bisa di gunakan oleh Admin Sasha!`, id)
            if (!isVipUser) return tobz.reply(from, `Perintah ini hanya untuk Owner, Admin, dan User VIP Sasha!.`, id)
			if (!isNsfw) return tobz.reply(from, `command/Perintah NSFW belum di aktifkan di group ini!`, id)
            if (!isGroupMsg) return tobz.reply(from, `Perintah ini hanya bisa di gunakan dalam group!`, id)
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik #limit Untuk Mengecek Kuota Limit Kamu`, id)
            try {
            ...
            break
        case `${prefix}xvideos`:
            if (!isVipUser) return tobz.reply(from, `Perintah ini hanya bisa di gunakan oleh Owner, Admin, dan User VIP Sasha!`, id)
            if (!isNsfw) return tobz.reply(from, `command/Perintah NSFW belum di aktifkan di group ini!`, id)
            if (!isGroupMsg) return tobz.reply(from, `Perintah ini hanya bisa di gunakan dalam group!`, id)
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik #limit Untuk Mengecek Kuota Limit Kamu`, id)
