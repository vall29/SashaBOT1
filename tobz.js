    ...
    try {
        ...
        // FUNCTION
                ...
                // MRHRTZ
						...
						if (chats.match("anjing") || chats.match("gblk") || chats.match("bgst") || chats.match("tolol") || chats.match("kntl") || chats.match("kontol") || chats.match("memek") || chats.match("asu") || chats.match("mmk") || chats.match("ngentod") || chats.match("babi") || chats.match("monyet") || chats.match("ajg")) {
                        ...
                        }).catch(() => tobz.sendText(from, `Untung Gua Bukan Admin, Kalo Jadi Admin Udah Aku Kick Tuh! 😑`))
                    } else {
                ...
                if(body === `${prefix}mute` && isMuted(chatId) == true){
                    if(isGroupMsg) {
                        if (!isAdmin) return tobz.reply(from, 'Maaf, perintah ini hanya dapat dilakukan oleh admin Sasha!', id)
				...
				if (chats.match("sasha") || chats.match("Sasha") || chats.match("SASHA") || chats.match("bot") || chats.match("Bot") || chats.match("BOT")) {
					const bls = balas[Math.floor(Math.random() * (balas.length))]
					await tobz.reply(from, `*${bls}* \n\n`, id)
				}

                if (body === `${prefix}unbanchat`) {
                    if (!isOwner) return tobz.reply(from, 'Maaf, perintah ini hanya dapat dilakukan oleh Owner Sasha!', id)
        ...
        case `${prefix}banchat`:
            ...
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner Sasha!', id)
            ...
            break
		case `prefix`:
			tobz.reply(from, `*Sasha is Use ( ${prefix} ) Prefix!.* 
...
_Contoh: ${prefix}menu_`, id)
		...
		case `${prefix}setto`:
			...
			if (args.length === 1) return tobz.reply(from, `*Kirim Perintah ${prefix}setto [prefix baru]*. 
Contoh: ${prefix}setto #`, id)
		...
		case `${prefix}bukagc`:
		case `${prefix}open`:
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh AdminSasha!', id)
        ...
        case `${prefix}sgif`:
            if (isMedia) {
                if (mimetype === 'video/mp4' && message.duration < 10 || mimetype === 'image/gif' && message.duration < 10) {
                    ...
                    tobz.reply(from, `[❗] Kirim video dengan caption *${prefix}stickerGif* max 10 sec!'`, id)
                )
            }
            break
...
${desc}`)
        ...
        case `${prefix}simi`:
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Admin Sasha!', id) // Hanya Admin yang bisa mengaktifkan
            if (args.length === 1) return tobz.reply(from, 'Pilih enable atau disable!', id)
            if (args[1].toLowerCase() === 'enable') {
                simi_.push(chat.id)
...
🌐️ *URL*: ${url}`
        ...
        case `${prefix}quran`:
            if (!isGroupMsg) return tobz.reply(from, `Perintah ini hanya bisa di gunakan dalam group!`, id)
            if (!isVipUser) return tobz.reply(from, 'Fitur ini hanya dapat digunakan oleh User Vip Sasha!', id)
			if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik @limit Untuk Mengecek Kuota Limit Kamu`, id)
            ...
            try {
                ...
                const ytvh2 = await ytvh.json()
                 if (ytvh2.status == false) {
                    ...
                    if (Number(ytvh2.result.size.split(' MB')[0]) > 50.00) return tobz.reply(from, `Maaf durasi video sudah melebihi batas maksimal 30 MB!`, id)
            ...
            try {
                ...
                const vhtearyt33 = await vhtearyt3.json()
                 if (vhtearyt33.status == false) {
                    ...
                    if (Number(vhtearyt33.result.size.split(' MB')[0]) >= 30.00) return tobz.reply(from, 'Maaf durasi audio sudah melebihi batas maksimal 10 MB!', id)
        ...
        case `${prefix}nhentai`:
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isNsfw) return tobz.reply(from, 'command/Perintah NSFW belum di aktifkan di group ini!', id)
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik _limit Untuk Mengecek Kuota Limit Kamu`, id)
            
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah *_nhentai* [ Query ]')
            const quernh = body.slice(9)
            tobz.reply(from, mess.wait, id)
            try {
                const resnh = await fetch(`https://mhankbarbar.herokuapp.com/api/nhentai?type=search&query=${encodeURIComponent(quernh)}&apiKey=${barbarkey}`)
                if (!resnh.ok) throw new Error(`unexpected response ${resnh.statusText}`)
                const jsonnh = await resnh.json()
                const { result } = await jsonnh
                let berhitung = 1
                let xixixi = `*「 NHENTAI 」*\n\n*Hasil Pencarian : ${quernh}*\n\n─────────────────\n\nKetik _getnhentai [ Angka ] untuk mengambil ID, Contoh : _getnhentai 2\n`
                for (let i = 0; i < result.length; i++) {
                    xixixi += `\n─────────────────\n\n*Urutan* : ${berhitung+i}\n*Title* : ${result[i].title}\n*Bahasa* : ${result[i].lang}\n*Perintah download* : *_getnhentai ${result[i].id}*\n`
                }
                    xixixi += `\n\n`
        ...
        case `${prefix}getnhentai`:
            ...
            try {
                if (quotedMsg && quotedMsg.type == 'image') {
                    if (args.length === 1) return tobz.reply(from, 'Kirim perintah *_getnhentai [ Id Download ]*, untuk contoh silahkan kirim perintah *_readme*')
                    if (!Number(args[1])) return tobz.reply(from, `*Apabila ditag hanya cantumkan nomer urutan bukan ID Download!*\nContoh : *_getnhentai 1*`, id)
                    ...
                    tobz.reply(from, mess.wait, id)
                    const mhanknh35 = await get.get(`http://mhankbarbar.herokuapp.com/api/nhentai?type=download&nuklir=${pilur[args[1]]}&apiKey=${barbarkey}`).json()      
                    ...
                    tobz.reply(from, mess.wait, id)
                    const mhankyt35 = await fetch(`http://mhankbarbar.herokuapp.com/api/nhentai?type=download&nuklir=${args[1]}&apiKey=${barbarkey}`)
        ...
        case `${prefix}video`:
			...
			if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik _limit Untuk Mengecek Kuota Limit Kamu`, id)
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah *_video* [video]')
            ...
            try {
                ...
                let xixixai = `*Hasil pencarian dari ${querv}*\n\nKetik _getvideo [angka] untuk mengambil ID, Contoh : _getvideo 2\n`
                for (let i = 0; i < result.length; i++) {
        ...
        case `${prefix}getvideo`:
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isVipUser) return tobz.reply(from, 'Fitur ini hanya dapat digunakan oleh User Vip Sasha!', id)
			if (args.length === 1) return tobz.reply(from, `Kirim perintah *@getvideo* [ Id Download ], untuk contoh silahkan kirim perintah *@readme*`, id)
            try {    
            if (quotedMsg && quotedMsg.type == 'image') {
                ...
                if (mhankyt45.status == false) {
                    ...
                    if (Number(mhankyt45.result.size.split(' MB')[0]) > 30.00) return tobz.sendFileFromUrl(from, imgUrl, `thumb.jpg`, `*「 YOUTUBE VIDEO DOWNLOADER 」*\n\n*Title* : ${title}\n*Ext* : MP3\n*Filesize* : ${size}\n*Link* : ${UrlVideo}\n\n_Untuk durasi lebih dari batas disajikan dalam bentuk link_`, id)
                ...
                if (args.length === 1) return tobz.reply(from, `Kirim perintah *@getvideo [ Id Download ]*, untuk contoh silahkan kirim perintah *@readme*`)
		...
		case `${prefix}musik`:
		    ...
		    if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik _limit Untuk Mengecek Kuota Limit Kamu`, id)
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah *_musik* [lagu]')
            ...
            try {
                ...
                let berhitung = 1
                let xixixi = `*Hasil pencarian dari ${quer}*\n\nKetik _getmusik [angka] untuk mengambil ID, Contoh : _getmusik 2\n`
                for (let i = 0; i < result.length; i++) {
        ...
        case `${prefix}getmusic`:
            ...
            try {
                if (quotedMsg && quotedMsg.type == 'image') {
                    if (args.length === 1) return tobz.reply(from, `Kirim perintah *@getmusik [ Id Download ]*, untuk contoh silahkan kirim perintah *@readme*`)
                    ...
                    const mhankyt35 = await mhanky35.json()
                     if (mhankyt35.status == false) {
                        ...
                        if (Number(mhankyt35.result.size.split(' MB')[0]) >= 10.00) return tobz.sendFileFromUrl(from, imgUrl, `thumb.jpg`, `*「 YOUTUBE MUSIC DOWNLOADER 」*\n\n*Title* : ${title}\n*Ext* : MP3\n*Filesize* : ${size}\n*Link* : ${UrlMp3}\n\n*Untuk durasi lebih dari batas disajikan dalam bentuk link*`, id)
        ...
        case `${prefix}youtubesearch`:
            ...
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah *_youtubesearch [query]*, Contoh : _youtubesearch alan walker alone')
        ...
        case `${prefix}brainlysearch`:
            ...
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah *_brainlysearch [query]*, Contoh : *_brainlysearch Siapa itu Sasha?*')
        ...
        case `${prefix}shopee`:
            ...
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah *_shopee [query]*, Contoh : *_shopee HP Samsul a20*')
        ...
        case `${prefix}playstore`:
            ...
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah *_playstore [query]*, Contoh : *_playstore Mobile Legends*')
        ...
        case `${prefix}animesearch`:
            ...
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah *_animesearch [query]*, Contoh : _animesearch DXD (Hanya Bisa Satu Kata)')
...
➸ *Total Rekaman:* ${recording}`
        ...
        case `${prefix}instagram`:
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik @limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah *${previx}ig [ Link Instagram ]* untuk contoh silahkan kirim perintah *@readme*`)
...
➸ *Wilayah:* +${region_name}`
        ...
        case `${prefix}brainly`:
                if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
                if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik #limit Untuk Mengecek Kuota Limit Kamu`, id)
                if (args.length === 1) return tobz.reply(from, `Kirim perintah ${fix}brainly [queru]\nContoh : ${predix}brainly keajaiban dunia`, id)
                    try {
        ...
        // ADMIN & OWNER
        case `${prefix}bc`:
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya untuk Owner Sasha!', id)
            if (quotedMsg && quotedMsg.type == 'image') {
            const mediaData = await decryptMedia(quotedMsg, uaOverride)
            const imageBase64 = `data:${quotedMsg.mimetype};base64,${mediaData.toString('base64')}`
            ...
            for (let ids of chatz) {
                ...
                if (!cvk.isReadOnly) 
                await tobz.sendFile(ids, `${imageBase64}`,'bc.jpg', `[ *Sasha BroadCast* ]\n\n${msg}`)
            }
            } else {
            let msg = body.slice(4)
            const chatz = await tobz.getAllChatIds()
            for (let ids of chatz) {
                var cvk = await tobz.getChatById(ids)
                if (!cvk.isReadOnly) 
                await tobz.sendText(ids, `[ *Sasha BroadCast* ]\n\n${msg}`)
            }
        }
		...
		case `${prefix}p`:
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Owner Sasha', id)
        ...
        case `${prefix}okickall`:
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya untuk Admin Sasha', id)
            if (!isBotGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
        ...
        case `${prefix}oadd`:
            ...
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin Sasha', id)
            if (!isBotGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
            try {
        ...
        case `${prefix}oleave`:
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin Sasha', id)
            await tobz.sendText(from,'SASHA DIPERINTAHKAN KELUAR OLEH OWNER!!').then(() => tobz.leaveGroup(groupId))
            break
        case `${prefix}leave`:
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh admin Sasha', id)
        ...
        case `${prefix}getss`:
            ...
            tobz.sendFile(from, sesPic, 'session.png', 'Nih SAYANG!', id)
            break
		...
		case `${prefix}listbadword`:
			...
			await tobz.reply(from, lbw, id)
			break
        case `${prefix}limit`:
			if (!isAdmin) return tobz.reply(from, 'Jika ingin cek limit bilang ke Admin atau Owner Sasha ya Kak...', id)
            var found = false
            const limidat = JSON.parse(fs.readFileSync('./lib/database/limit.json'))
                for (let lmt of limidat) {
                    if (lmt.id === mentionedJidList[0]) {
                        let limitCounts = limitCount - lmt.limit
                        if (limitCounts <= 0) return tobz.sendTextWithMentions(from, `Limit request @${mentionedJidList[0].replace('@c.us', '')} sudah habis\n\n_Note : Limit akan direset setiap jam 09:00!_`, id)
                        tobz.sendTextWithMentions(from, `Sisa limit request @${mentionedJidList[0].replace('@c.us', '')} tersisa : *${limitCounts}*\n\n_Note : Limit akan direset setiap jam 09:00!_`, id)
                        found = true
                    }
                }
                if (found === false) {
                    ...
                    tobz.sendTextWithMentions(from, `Sisa limit request @${mentionedJidList[0].replace('@c.us', '')} tersisa : *${limitCount}*\n\n_Note : Limit akan direset setiap jam 09:00!_`, id)
				}
		            break
		case `${prefix}addlimit`:
			if (!isAdmin) return tobz.reply(from, 'Maaf kak, hanya untuk Admin Sasha.', id)
			var found = false;
                    Object.keys(limit).forEach((i) => {
		...
		case `${prefix}addbadword`:
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner Sasha!', id)
			if (!isGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Admin group', id)
			if (args.length == 1) return tobz.reply(from, `Kirim perintah ${prefix}addbadword [kata kasar]. contoh ${prefix}addbadword bego`,id)
                const bw = body.slice(12)
				bad.push(bw)
			fs.writeFileSync('./lib/database/bad.json', JSON.stringify(bad))
                tobz.reply(from, 'Success Menambahkan Bad Word!', id)
				break
		case `${prefix}delbadword`:
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner Sasha!', id)
			if (!isGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Admin group', id)
			if (args.length == 1) return tobz.reply(from, `Kirim perintah ${prefix}delbadword [kata kasar]. contoh ${prefix}delbadword bego`,id)
				let dbw = body.slice(12)
				bad.splice(dbw)
				fs.writeFileSync('./lib/database/bad.json', JSON.stringify(bad))
				tobz.reply(from, 'Success Menghapus BAD WORD!', id)
			break
		case `${prefix}addvip`:
			if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Admin Sasha', id)
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner Sasha!', id)
				...
				tobz.reply(from, 'Success Menambahkan User VIP Sasha!', id)
				}
            break
		case `${prefix}delvip`:
			if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Admin Sasha', id)
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner Sasha!', id)
				...
				VipUser.splice(dv, 1)
				fs.writeFileSync('./lib/database/VipUser.json', JSON.stringify(VipUser))
				tobz.reply(from, 'Success Menghapus User VIP Sasha!', id)
			break
		case `${prefix}addadmin`:
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner Sasha!', id)
                ...
                tobz.reply(from, 'Success Menambahkan Admin Sasha!', id)
				}
            break
        case `${prefix}deladmin`:
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner Sasha!', id)
                ...
                fs.writeFileSync('./lib/database/admin.json', JSON.stringify(adminNumber))
                tobz.reply(from, 'Success Menghapus Admin Sasha!', id)
            break
        case `${prefix}block`:
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner Sasha!', id)
            for (let i = 0; i < mentionedJidList.length; i++) {
        ...
        case `${prefix}bugreport`:
            ...
            if(isGroupMsg){
                ...
                tobz.reply(from, 'Masalah telah di laporkan ke owner BOT, laporan palsu/main2 tidak akan ditSashapi.' ,id)
            }else{
                tobz.sendText(ownerNumber, `*[BUG REPORT]*\n*WAKTU* : ${time}\nNO PENGIRIM : wa.me/${sender.id.match(/\d+/g)}\n\n${bug}`)
                tobz.reply(from, 'Masalah telah di laporkan ke owner BOT, laporan palsu/main2 tidak akan ditSashapi.', id)
            }
            break
		case `${prefix}getpp`:
			if (!isGroupMsg) return tobz.reply(from, 'Hanya untuk di grup!.', id)
				var pik = await tobz.getProfilePicFromServer(mentionedJidList[0])
				await tobz.sendFileFromUrl(from, pik, 'pik.jpg', 'Nih Kak...', id)
				break
		case `${prefix}getprofile`:
	    case `${prefix}you`:
			if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh AdminSasha!', id)
        ...
        case `${prefix}menu`:
        case `${prefix}help`:
            tobz.reply(from, help)
            break
		case `${prefix}intro`:
		    tobz.sendText(from, intro)
            break
        case `${prefix}sashagroup`:
            tobz.reply(from, `Link Group Sasha : https://chat.whatsapp.com/EF1Gm9Uyu2oF1fb57HAHLJ Jangan Lupa Join Ya Kak ${pushname}`, id)
            break
        ...
        case `${prefix}adminmenu`:
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin Sasha', id)
   ...
   *Text : ${swrt1}*`
		...
		case `${prefix}sarah`:
            ...
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah *_sarah [query]*\nContoh : *_sarah ZeroTwo*', id)
		...
		case `${prefix}slap`:
            ...
            tobz.sendTextWithMentions(from, `${prefix}` + jejiik + ' *slapped* ' + arg[1])
            break
        case `${prefix}hug`:
                arg = body.trim().split(' ')
		...
		case `${prefix}open`:
            ...
            break
		case `${prefix}setlink`:
			if(!isGroupMsg) return tobz.sendText(from, 'Maaf, Perintah ini hanya dapat digunakan didslam group!.', id)
			if(!isOwner) return tobz.sendTextWithMentions(from, `Maaf kak perintah ini hanya untuk Kak @${sender.id.replace('@c.us','')}!.`, id)
		    if(!isGroupAdmins) tobz.sendTextWithMentions(from, `Maaf kak perintah ini hanya bisa dilakukan oleh Admin Kak @${sender.id.replace('@c.us','')}!.`, id)
					await tobz.revokeGroupInviteLink(chat.id);
					tobz.sendText(from, 'Tautan undangan berhasil di tarik')
			break
		/*case `${prefix}setlink`:
			if(isGroupMsg){
			if(isOwner)
			...
			if(message.author == wkk) {
				try {
					...
					tobz.sendText(from, 'Tautan undangan berhasil di tarik')
						...
						tobz.reply(from, 'Sepertinya bot belum menjadi admin', id)
					}
			} else {
					tobz.reply(from, 'Maaf. fitur ini hanya untuk owner grup', id)
			} else {
				tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam grup', id)
			}
		    break*/
		case `${prefix}setname`:
			if(isGroupMsg){
			...
			try {
				const subject = body.slice(9)
							console.log(subject)
							tobz.setGroupTitle(from, subject, groupId)
				} catch {
						tobz.reply(from, 'Terjadi Kesalahan, tidak dapat mengubah title group!', message)
					   }
				} else {
						tobz.reply(from, 'Maaf, fitur ini hanya untuk owner grup', message)
				}
				}
			    } else {
				tobz.reply(from, 'Fitur ini hanya bisa gunakan dalam grup', message)
			}
		    break
		case `${prefix}setpic`:
			    if(isGroupMsg) {
				    ...
				    tobz.reply(from, 'Fitur ini hanya bisa di gunakan dalam grup', message)
			    }
		    break
		case `${prefix}setdesk`:
			if(isGroupMsg) {
				...
				if(message.author == wkk || message.author == '6281338888294@c.us') {
					try {
						...
						await tobz.setGroupDescription(from, `${desk}`)
					} catch {
						tobz.reply(from, 'Terjadi kesalahan, tidak dapat mengubah deskripsi grup', message)
					}
				}else{
					tobz.reply(from, 'Maaf, fitur ini hanya untuk owner grup', message)
				}
		...
		case `${prefix}getlink`:
            ...
            tobz.sendLinkWithAutoPreview(from, inviteLink, `\nLink group *${name}*`)
   			break
		case `${prefix}blackpink`:
            if (!isGroupMsg) return tobz.sendText(from, 'Maaf kak, hanya untuk didalam grup', id)
            if (args.length === 1) return tobz.reply(from, `kirim perintah ${prefix}blackpink [text]. Contoh: *${prefix}blackpink Sasha*`, id)
			if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis.`, id)
			const black = body.slice(11)
            const pink = `https://api.vhtear.com/blackpinkicon?text=${black}&apikey=${vhtearkey}`
            if (pink.error) return tobz.reply(from, pink.error, id)
            tobz.reply(from, 'Tunggu sebentar!.', id)
            await tobz.sendFileFromUrl(from, pink, 'blackpink.jpg', 'Yahahaha EMBLEK EMPINK :v', id)
            await limitAdd(serial)
			break
		case `${prefix}thunder`:
			if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya dapat digunakan didalam Group', id)
			if (!isVipUser) return tobz.reply(from, 'Perintah ini hanya untuk User VIP👑 Sasha', id)
            if (args.length === 1)return tobz.reply(from, `Kirim perintah ${prefix}thunder [text].\nContoh: ${prefix}thunder Sasha`, id)
			if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis.`, id)
			const th = body.slice(9)
			const tu = `https://api.vhtear.com/thundertext?text=${th}&apikey=${vhtearkey}`
			tobz.reply(from, 'Tunggu sebentar!.', id)
			await tobz.sendFileFromUrl(from, tu, 'Thunder.jpg', 'Nih Kak...', id)
            await limitAdd(serial)
            break
		case `${prefix}nyimak`:
            if(!isGroupMsg) return tobz.reply(from, 'Hanya untuk di dalam group', id)
            if(!quotedMsg) return tobz.reply(from,'reply pesan bot')
            try{
                ...
                tobz.sendTextWithMentions(from, `Ciee, Ngeread doang... Jawab boss!! Ini grup WA Boss!! bukan KORAN!!!.\n${list}`, id)
            } catch(err) {
                console.log(err)
                tobz.reply(from,'Maaf, Belum ada yang membaca pesan Sasha', id)
            }
            break
		case `${prefix}xxx`:
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
			...
			if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik _limit Untuk Mengecek Kuota Limit Kamu`, id)
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah *${prefix}xxx* [video]`, id)
            const querb = body.slice(5)
            tobz.reply(from, mess.wait, id)
            try {
                const resmusn = await axios.get(`https://api.vhtear.com/xxxsearch?query=${querb}&apikey=+${vhtearkey}`)
                if (!resmusn.ok) throw new Error(`unexpected response ${resmusn.statusText}`)
                const jsonsercmun = await resmusn.json()
                let berhitung1 = 1
                const { result } = await jsonsercmun
                let xixixai = `*Hasil pencarian dari ${querb}*\n\nKetik ${prefix}getxxx [angka] untuk mengambil ID, Contoh : ${prefix}getxxx 1\n`
                for (let i = 0; i < result.length; i++) {
                    xixixai += `\n═════════════════\n\n*Urutan* : ${i+1}\n*Title* : ${result[i].title}\n*Channel* : ${result[i].channel}\n*Durasi* : ${result[i].duration}\n*Perintah download* : _getvideo ${result[i].id}\n`
                }
                    xixixai += `\n\n`
                ...
                await tobz.sendFileFromUrl(from, result[0].image, 'thumbsec.jpg', xixixai, id)
            } catch (err){
                console.log(err)
            }
            break
        ...
        case `${prefix}vgetvideo':
            ...
            if (quotedMsg && quotedMsg.type == 'image') {
                ...
                if (mhankyt45.status == false) {
                    ...
                    const { title, ext, imgUrl, size, UrlVideo } = await mhankyt45.result
                    if (Number(mhankyt35.result.size.split(' MB')[0]) > 30.00) return tobz.sendFileFromUrl(from, imgUrl, `thumb.jpg`, `*「 YOUTUBE VIDEO DOWNLOADER 」*\n\n*Title* : ${title}\n*Ext* : MP3\n*Filesize* : ${size}\n*Link* : ${UrlVideo}\n\n_Untuk durasi lebih dari batas disajikan dalam bentuk link_`, id)
                        const captions = `*「 YOUTUBE VIDEO DOWNLOADER 」*\n\n*Title* : ${title}\n*Ext* : MP4\n*Filesize* : ${size}\n\n*Silahkan tunggu file media sedang dikirim mungkin butuh beberapa menit*`                  
                        tobz.sendFileFromUrl(from, imgUrl, `thumb.jpg`, captions, id)
                        await tobz.sendFileFromUrl(from, UrlVideo, `${title}.mp4`, `Video telah terkirim ${pushname}`, id).catch(() => tobz.reply(from, mess.error.Yt3, id))
                        await limitAdd(serial)
		...
		case `${prefix}vmusik`:
            ...
            break
        case `${prefix}vgetmusik':
        case `${prefix}vgetmusic':
            if (!isAdmin) return tobz.reply(from, `Perintah ini hanya bisa di gunakan oleh Admin Sasha!`, id)
            if (!isGroupMsg) return tobz.reply(from, `Perintah ini hanya bisa di gunakan dalam group!`, id)
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik @limit Untuk Mengecek Kuota Limit Kamu`, id)
            try {
		...
		case `${prefix}klasmen`:
            ...
            await tobz.sendTextWithMentions(from, textKlas)
			break
		default:
            if (!isGroupMsg) return tobz.reply(from, `Jika Ingin Menggunakan Bot Harap Masuk Ke Dalam Grup Sasha, Link Ada Di Bio atau Bisa Mengetik ${prefix}sashagroup!\nJika Ingin Sewa Bot atau Bikin Bot Harap Ketik *${prefix}iklan*`, id)
